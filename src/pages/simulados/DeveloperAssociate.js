...
const AlternativeItem = styled.li`
  ...
  &:hover {
    ...
    background-color: var(--aws-orange-50);
  }
  ...
  &.correct {
    ...
    background-color: var(--aws-green-50);
  }
  
  &.incorrect {
    border-color: var(--aws-red);
    background-color: var(--aws-red-50);
  }
...
const DeveloperAssociate = () => {
  ...
  const checkAnswer = (questionIndex, selectedAnswer) => {
    const question = developerAssociateQuestions[questionIndex];
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < developerAssociateQuestions.length - 1) {
  ...
  const currentQ = developerAssociateQuestions[currentQuestion];
  ...
  return (
    <ExamProtection>
      <SimuladoContainer>
        ...
        <Header>
          <QuestionCounter>
            {currentQuestion + 1} / {developerAssociateQuestions.length}
          </QuestionCounter>
          <Timer>
        ...
        <div style={{ display: 'flex', gap: 'var(--spacing-4)', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {currentQuestion === developerAssociateQuestions.length - 1 ? 'Finalizar' : 'Próxima →'}
          </Button>
...
export default DeveloperAssociate;
