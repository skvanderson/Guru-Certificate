...
const SolutionsArchitect = () => {
  ...
  const checkAnswer = (questionIndex, selectedAnswer) => {
    const question = solutionsArchitectQuestions[questionIndex];
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < solutionsArchitectQuestions.length - 1) {
  ...
  const finishExam = useCallback(() => {
    ...
    const totalQuestions = solutionsArchitectQuestions.length;
    ...
    Object.keys(answeredQuestions).forEach(questionIndex => {
      if (!answeredQuestions[questionIndex]) {
        const questionId = solutionsArchitectQuestions[parseInt(questionIndex)].id;
        wrongQuestionIds.push(questionId);
        
        // Extrair tópico do título
        const question = solutionsArchitectQuestions[parseInt(questionIndex)];
    ...
    if (currentUser) {
      try {
        saveExamResult(currentUser.id, 'solutions-architect', result);
      } catch (error) {
  ...
  return (
    <ExamProtection>
      <SimuladoContainer>
        ...
        <Header>
          <QuestionCounter>
            {currentQuestion + 1} / {solutionsArchitectQuestions.length}
          </QuestionCounter>
          <Timer>
        ...
        <div style={{ display: 'flex', gap: 'var(--spacing-4)', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {currentQuestion === solutionsArchitectQuestions.length - 1 ? 'Finalizar' : 'Próxima →'}
          </Button>
